...
class Framework {
    ...
    private static function initConst() {
        ...
        define('FRAWORK_PATH',ROOT_PATH,'Framework'.DS);    //Framework目录
        ...
        define('CORE_PATH',APP_PATH,'Core'.DS);     //Core目录
        define('LIB_PATH',APP_PATH,'Lib'.DS);       //Lib目录
    }
    /*
     * 导入配置文件
     */
    ...
    private static function initRoutes() {
        $p = isset($_REQUEST['p'])?$_REQUEST['p']:$GLOBALS['config']['app']['default_platform'];    //当前平台
        $c = isset($_REQUEST['c'])?$_REQUEST['c']:$GLOBALS['config']['app']['default_controller'];  //当前控制器
        $a = isset($_REQUEST['a'])?$_REQUEST['a']:$GLOBALS['config']['app']['default_action'];      //当前方法
        define('PLATFORM',$p);
     ...
     * 注册自定义加载类函数
     */
    private function initRegisterAutoLoad() {
    ...
    private static function initDispatch() {
        ...
        $controller = new $class_name();        //实例化控制器
